...
if($uname != "")
{
	...
	if($USERID > 0)
	{
		...
		$query = "select A.PID, A.ptitle, A.pic, A.pkey, A.price, A.youtube, B.USERID, B.username, B.fname, B.lname, B.profilepicture, C.bname from posts A, members B, boards C, posts_fav D WHERE A.active='1' AND A.USERID=B.USERID AND A.BID=C.BID AND A.PID=D.PID AND D.USERID='".mysql_real_escape_string($USERID)."' order by A.points desc, A.viewcount desc, A.PID desc limit 50"; 
...
STemplate::display('profilelikes.tpl');
