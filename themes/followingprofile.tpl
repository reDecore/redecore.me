<div class="ColumnContainer" style="margin-top: {if $smarty.session.USERID eq ""}160{else}80{/if}px;">
    ...
    <div class="BoardTitle">
		<h1>{$u.username|stripslashes}</h1>
		<div class="desc"></div>
        <div class="BoardMeta sysBoardItemContainer">
            <div class="BoardUsers">
            	<a href="{$baseurl}/{$u.username|stripslashes}" class="ImgLink"><img src="{$murl}/thumbs/{insert name=get_member_profilepicture value=var assign=bpp profilepicture=$u.profilepicture}{$bpp}" alt="{$u.username|stripslashes}" title="{$u.username|stripslashes}"/></a>
            	<span class="BoardUserName"><a href="{$baseurl}/{$u.username|stripslashes}">{if $use_username eq "1"}{$u.username|stripslashes}{else}{$u.fname|stripslashes} {$u.lname|stripslashes}{/if}</a></span>
            </div>
            <div class="BoardStats"><span><a href="{$baseurl}/{$u.username|stripslashes}">{$tb} {$lang176}</a></span>, <span><a href="{$baseurl}/{$u.username|stripslashes}/pins">{$tp} {$lang175}</a></span></div>
            <div class="BoardButton">                
                {if $smarty.session.USERID ne ""}
                    {if $smarty.session.USERID ne $u.USERID}
                    {insert name=is_folm assign=isfolo USERID=$u.USERID}
                    <div class="sysBoardFollowButton {if $isfolo eq "1"}hidden{/if}" id="SFADD{$u.USERID}" onclick="ATF('{$u.USERID}', 'SFADD{$u.USERID}', 'SFREM{$u.USERID}');"><a class="Button12 Button WhiteButton" href="javascript:void(0)"><strong>{$lang130}</strong><span></span></a></div>
                    <div class="sysBoardUnFollowButton {if $isfolo eq "0"}hidden{/if}" id="SFREM{$u.USERID}" onclick="RFF('{$u.USERID}', 'SFREM{$u.USERID}', 'SFADD{$u.USERID}');"><a class="Button12 Button WhiteButton disabled" href="javascript:void(0)"><strong>{$lang131}</strong><span></span></a></div>
                    {/if}
                {else}
