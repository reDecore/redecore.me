<div class="pinPopup">
    ...
    <div class="WhiteContainer clearfix sysPinItemContainer" owner_id="{$pins.USERID}" pin_id="{$pins.pkey}">
        ...
        <div class="sysPinCmntList_Full PinComments">        
            ...
            <p class="nologged" style="padding-left:10px; padding-top:5px;"><a href="{$baseurl}/login">{$lang124}</a></p>
            {/if}
        </div>  
        {if $smarty.session.USERID ne ""}
        ...
        {/if}                                 
        <div class="PinActivity ZoomBoard sysBoardItemContainer" owner_id="{$pins.USERID}" owner_login="{$pins.username|stripslashes}" board_id="{$pins.BID}">        
        	{if $smarty.session.USERID ne ""}
                {if $smarty.session.USERID ne $pins.USERID}
            ...
            {/if}            
            <p class="colorless">{$lang125}</p>
            <h4><a href="{$baseurl}/{$pins.username|stripslashes}/{insert name=get_seo_bname value=a assign=bname bname=$pins.bname}{$bname|stripslashes}">{$pins.bname|stripslashes}</a></h4>
            {if $bp|@count GT "0"}
            {section name=i loop=$bp}
            <a class="CommenterImage" href="/pin/{$bp[i].pkey}" onclick="return Nav.go(this, 'pin')"><img alt="{$bp[i].ptitle|stripslashes}"  src="{$purl}/t/t-{$bp[i].pic}" /></a>
            {/section}
            {/if}
        </div>
        <div class="PinActivity ZoomUrl BorderBottom">
            <div class="ZoomOrigin">
            	{if $smarty.session.USERID ne ""}
                    {if $smarty.session.USERID ne $pins.USERID}
                    {if $isfolm GTE "0"}
                ...
                <div class="sysBoardFollowAllButton "><a href="{$baseurl}/login" class="Button12 Button OrangeButton"><strong>{$lang130}</strong><span></span></a></div>
                {/if}
            	<p class="colorless">{$lang126}</p>
            	<h4><a href="{$baseurl}/{$o_name|stripslashes}">{if $use_username eq "1"}{$o_name|stripslashes|mb_truncate:50:"...":'UTF-8'}{else}{$o_fname|stripslashes}{/if}</a></h4>
                {if $op|@count GT "0"}
            	{section name=i loop=$op}
                <a class="link" href="/pin/{$op[i].pkey}" onclick="return Nav.go(this, 'pin')"><img alt="{$op[i].ptitle|stripslashes}"  src="{$purl}/t/t-{$op[i].pic}" /></a>
                {/section}
            	{/if}
            </div>
            {if $csource ne ""}
            <div class="ZoomSource">
            	<p class="colorless">{$lang127}</p>
                <h4><a href="{$baseurl}/source/{$csource|stripslashes}">{$csource|stripslashes}</a></h4>
                {if $sim|@count GT "0"}
            	{section name=i loop=$sim}
                <a href="{$baseurl}/source/{$csource|stripslashes}" class="link"><img src="{$purl}/t/t-{$sim[i].pic}"/></a>
                {/section}
            	{/if}
            </div>    
            {/if}    
        ...
        {if $rp|@count GT "0"}
        <div class="PinActivity ZoomRepins">
            ...
            <div class="userRepin">
                ...
                <a class="CommenterName" href="{$baseurl}/{$rp[i].username|stripslashes}">{if $use_username eq "1"}{$rp[i].username|stripslashes}{else}{$rp[i].fname|stripslashes} {$rp[i].lname|stripslashes}{/if}</a><br/>
