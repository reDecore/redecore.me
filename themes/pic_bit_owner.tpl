        ...
        {/if}
            <div class="sysPinActionButtonsContainer actions hidden">
                ...
                <a href="{$baseurl}/edit_pin?id={$pins[i].PID}" class="Button Button11 WhiteButton"><strong>{$lang223}</strong><span></span></a>
                <a href="javascript:void(0)" {literal}onclick="App.ajaxDialog('{/literal}{$baseurl}{literal}/delete_create_popup?id={/literal}{$pins[i].PID}{literal}', {id: 'sysPinDeletePopup', width: '400px'})"{/literal} class="Button Button11 WhiteButton"><strong>{$lang224}</strong><span></span></a>
            ...
            <div class="convo attribution clearfix">
                <a href="{$baseurl}/{$smarty.session.USERNAME|stripslashes}" class="ImgLink"><img src="{$murl}/thumbs/{insert name=get_member_profilepicture value=var assign=pinpp profilepicture=$smarty.session.PP}{$pinpp}" alt="{$smarty.session.USERNAME|stripslashes}" /></a>
                <p>
                    <a href="{$baseurl}/{$smarty.session.USERNAME|stripslashes}" >{if $use_username eq "1"}{$pins[i].username|stripslashes|mb_truncate:50:"...":'UTF-8'}{else}{$smarty.session.FNAME|stripslashes} {$smarty.session.LNAME|stripslashes}{/if}</a> {$lang100} <a href="{$baseurl}/{$smarty.session.USERNAME|stripslashes}/{insert name=get_seo_bname value=a assign=bname bname=$pins[i].bname}{$bname|stripslashes}" title="{$pins[i].bname|stripslashes}">{$pins[i].bname|stripslashes|mb_truncate:100:"...":'UTF-8'}</a>
                </p>
            </div>
