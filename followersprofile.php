...
if($uname != "")
{
	...
	if($USERID > 0)
	{
		STemplate::assign('pagetitle',stripslashes($u[0]['username'])." ".stripslashes($lang['180']));
		
		$query = "select A.USERID, B.username, B.fname, B.lname, B.profilepicture from followm A, members B WHERE B.status='1' AND A.USERID=B.USERID AND A.ISFOL='".mysql_real_escape_string($USERID)."' order by rand() limit 50"; 
...
STemplate::display('followersprofile.tpl');
