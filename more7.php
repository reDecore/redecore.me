...
if($q > 0)
{
	...
	$query = "select A.PID, A.ptitle, A.pic, A.pkey, A.price, A.youtube, B.USERID, B.username, B.fname, B.lname, B.profilepicture, C.bname from posts A, members B, boards C, posts_fav D WHERE A.active='1' AND A.USERID=B.USERID AND A.BID=C.BID AND A.PID=D.PID $addme order by A.points desc, A.viewcount desc, A.PID desc limit $offset, 10"; 
