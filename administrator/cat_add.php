...
if($_POST['submitform'] == "1")
{
	...
	if($seo != "")
	{
		$query="SELECT count(*) as total FROM categories WHERE seo='".mysql_real_escape_string($seo)."'";
	...
	{
		$error = "Error: Please select an image for your category.";
		Stemplate::assign('error',$error);
	}
	else
	{
		$sql = "insert categories set name='".mysql_real_escape_string($cname)."', seo='".mysql_real_escape_string($seo)."', example='".mysql_real_escape_string($cmsg)."'";
		$conn->execute($sql);
